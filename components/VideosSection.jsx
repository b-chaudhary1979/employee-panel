...
export default function VideosSection({ onFavourite }) {
  ...
  const [confirmDelete, setConfirmDelete] = useState(null); // video to delete
  const [notification, setNotification] = useState("");
  const [comments, setComments] = useState([]); // To store comments for the current video
  ...
  const [videos, setVideos] = useState([]); // To be populated with real data

  // Use local videos state or fallback to empty array
  const allVideos = videos && Array.isArray(videos) ? videos : [];
  // Filtered data
  const filteredVideos = allVideos.filter((vid) => {
    const q = search.toLowerCase();
    return (
      (vid.title && vid.title.toLowerCase().includes(q)) ||
      (vid.uploadedAt && vid.uploadedAt.toLowerCase().includes(q)) ||
      (vid.submitterName && vid.submitterName.toLowerCase().includes(q))
    );
  });

  const toggleFavourite = (vid) => {
    if (onFavourite) onFavourite(vid, true);
  };
  ...
  useEffect(() => {
    if (modal && modal.video && modal.video.id) {
  ...
  // --- Firestore real-time videos fetching (updated for individual document structure) ---
  useEffect(() => {
    if (!companyId) return;
    let unsubscribe;
  ...
  const handleFeedbackSubmit = async () => {
    if (!modal || !modal.feedback || !modal.video?.id) return;

    try {
      const result = await addComment(modal.video.id, modal.feedback, "videos");
      if (result.success) {
        setNotification("Feedback submitted!");
        setModal((prev) => ({ ...prev, feedback: "" }));
      } else {
  ...
  return (
    <div>
      <div className="mb-6 text-left">
        <h2 className="text-2xl font-bold text-[#16a34a]">Videos</h2>
        <p className="text-gray-500 text-base mt-1">
          Watch and organize your video files.
        </p>
        ...
        {filteredVideos.length === 0 ? (
          ...
          filteredVideos.map((vid) => (
            <div
              ...
              onClick={() => setModal({ video: vid, feedback: "" })}
            >
              {/* Star icon */}
              <button
                className="absolute top-2 right-2 z-10 p-0.5 bg-[#16a34a] rounded-full shadow-md hover:bg-[#5b21b6] transition"
                onClick={(e) => {
                  e.stopPropagation();
                  toggleFavourite(vid);
                }}
              ...
              <div className="text-gray-500 text-xs mt-1">
                {vid.date ||
                  (vid.uploadedAt
                    ? new Date(
                        vid.uploadedAt.seconds
                          ? vid.uploadedAt.seconds * 1000
                          : vid.uploadedAt
                      ).toLocaleDateString()
                    : "")}
              </div>
      ...
      {modal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 animate-fadeIn">
          ...
          {notification && (
            <div className="fixed top-8 left-1/2 -translate-x-1/2 z-[100] bg-gradient-to-r from-[#16a34a] to-[#a78bfa] text-white px-6 py-3 rounded-xl shadow-xl font-semibold flex items-center gap-2 text-base animate-slideDown">
              <svg
                xmlns="http://www.w3.org/2000/svg"
                fill="none"
                viewBox="0 0 24 24"
          ...
          >
            ...
            <div className="flex w-full justify-between items-center mb-4 p-6 pb-0">
              <div className="flex items-center gap-3">
                ...
                <span className="text-gray-700 font-bold text-base">
                  {modal.video.submitterName}
                </span>
                <span className="text-gray-700 text-sm">
                  {modal.video.date ||
                    (modal.video.uploadedAt
                      ? new Date(
                          modal.video.uploadedAt.seconds
                            ? modal.video.uploadedAt.seconds * 1000
                            : modal.video.uploadedAt
                        ).toLocaleDateString()
                      : "-")}
                </span>
            ...
            {/* Work Link Section */}
            <WorkLinkSection url={modal.video.cloudinaryUrl} />
            ...
            <div className="flex gap-3 w-full px-6 mb-6">
              ...
              <button
                onClick={() => handlePreview(modal.video)}
            ...
            <div className="flex gap-3 w-full px-6 mb-8">
              <a
                href={modal.video.cloudinaryUrl}
                download={modal.video.fileName || "video.mp4"}
                className="flex-1 flex items-center justify-center gap-2 bg-white border border-green-400 text-gray-700 rounded-full px-5 py-2 font-bold shadow hover:bg-green-50 active:scale-95 transition-all text-center"
              >
              ...
              <button
                ...
                onClick={() => setConfirmDelete(modal.video.cloudinaryPublicId)}
              >
                      ...
                      onClick={async () => {
                        ...
                        try {
                          const result = await deleteMedia({
                            ...modal.video,
                            id: modal.video.id, // pass actual Firestore doc ID
